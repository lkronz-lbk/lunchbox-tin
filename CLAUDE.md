...
- `netlify/functions/`: `api-auth.js` (magic links, sessions, delete), `api-household.js`
  ...
  (the numbers page at `/admin`, for `ADMIN_EMAILS`), `api-copy.js` (the wording editor at
  `/admin/copy`, for the same people: it saves rows in `site_copy` and pings the build hook;
  `scripts/copy.mjs --apply` folds them into `public/index.html` at build time, so the site
  stays static). `netlify/lib/copy.js` holds the parsing and the sanitiser;
  `netlify/lib/copy-fields.js` is generated by `npm run copy`.
