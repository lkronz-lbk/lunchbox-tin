...
<title>${esc(title)} · Lunch Sorted</title><meta name="color-scheme" content="light dark">
...
p{color:var(--ink-2);margin:0 0 14px;max-width:60ch} li{color:var(--ink-2);margin:0 0 6px}
...
.left{font:600 12px ui-monospace,monospace;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-3);margin:0 0 22px}
...
export default async function handler() {
  try {
    ...
    const ask = `<p>What we ask: use it for real for two weeks, and tell us what breaks or bugs you. The ? at the top of the app opens an email that already has the details we need in it.</p>
<p>Everything is on: the week planned in a minute, the shopping list, the kid&rsquo;s pick, the other parent&rsquo;s phone. It stays on for good, no card, ever.</p>`;
    ...
    return page('Join the beta', `<p class="eyebrow">Lunch Sorted beta</p><h1>Free forever, for the first ${esc(cap)} families to test it.</h1>${ask}
<a class="btn" href="/app/?beta=${encodeURIComponent(code)}">Join the beta</a>
<p class="left">${esc(left)} spot${left === 1 ? '' : 's'} left</p>
<p style="font-size:14px">Tap the button on the phone you pack lunches with. The app asks you to sign in with your email, no password, and the plan switches on for your household from there.</p>`);
  } catch (e) {
    console.error('beta', e);
    return page('Something went wrong', '<h1>Something went wrong on our side.</h1><p><a href="/beta">Try again</a></p>', 500);
  }
}
