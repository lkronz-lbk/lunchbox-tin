/* App Store screenshots. Serves public/ statically, walks the app through
   ...
   the ground color with a headline. Output in store/screenshots/.
...
for (let i = 0; i < 2; i++) { await page.locator('.tin [data-act="toggle"]').nth(i).click(); await wait(250); }   /* two things already in the bag; each checkmark re-renders, so fresh locators */
await page.evaluate(() => window.scrollTo(0, 0));
await shot('pack');
